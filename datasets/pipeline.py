"""
reference: https://github.com/muzairkhattak/ViFi-CLIP/blob/main/datasets/pipeline.py
"""
...
class MultiScaleCrop(RandomCrop):
    ...
    def __call__(self, results):
        ...
        img_h, img_w = results['img_shape'] # 256x341
        base_size = min(img_h, img_w)   # 256
        crop_sizes = [int(base_size * s) for s in self.scales]  # [256, 224, 192, 168]
...
class Resize:
    ...
    def __call__(self, results):
        ...
        img_h, img_w = results['img_shape']  # e.g., 240x320

        if self.keep_ratio:
            new_w, new_h = mmcv.rescale_size((img_w, img_h), self.scale)    # e.g., 256x341
        else:
...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        try:
            ...
            print("************ Failed to load video ", results['filename'])
            container = None
